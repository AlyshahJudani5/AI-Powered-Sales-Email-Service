"""
Sales Report Automation - Customized Version
Handles database connection, KPI calculation, AI insights, and report generation
Uses customized templates with dynamic theming from theme_config.json
"""
...
# Import from customized config (with dynamic theme loading)
from customized_config import TableConfig, COLORS, ThemeColors, BrandingConfig, IconsConfig, COUNTRY_CONFIG
...
from templates.customized_concise_email import generate_concise_email_template
from templates.customized_full_report_main import generate_full_report_template

load_dotenv()


def load_theme_config(client_name, config_path='theme_config.json'):
    """Load theme configuration for a specific client from JSON file"""
    try:
        with open(config_path, 'r') as f:
            all_themes = json.load(f)
        return all_themes.get(client_name, {})
    except Exception as e:
        logger.warning(f"Could not load theme_config.json for {client_name}: {e}")
        return {}
...
class SalesReportAutomation:
    ...
    def generate_and_send_report(self, specific_date=None, branch_ids='', recipients=None, is_branch=False):
        ...
        try:
            ...
            logger.info(f"Processing {scope} CUSTOMIZED report...")
            
            # Load theme configuration
            theme_config = load_theme_config(self.client_name)
            ...
            if self.send_email(concise_email_html, recipients, subject_title=email_subject_base):
                logger.info(f"CUSTOMIZED {scope} report email sent to {len(recipients)} recipients")
                return True
            else:
                logger.error(f"Failed to send CUSTOMIZED {scope} email")
                return False
                
        except Exception as e:
            logger.error(f"Error in customized generate_and_send_report for {scope}: {str(e)}")
            import traceback
    ...
    def run(self, specific_date=None):
        ...
        try:
            logger.info(f"==================================================")
            logger.info(f"STARTING CUSTOMIZED SALES REPORT: {self.client_name}")
            logger.info(f"==================================================")
